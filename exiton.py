...
for bhw in bhw_val:
    for i, p in enumerate(beads):
        ...
        # path = "/home/netanelb2/Desktop/Netanel/Research/exiton/anharmonic/boson3/bhw10/{}beads/".format(p)    # Anharmonic  # REMEMBER TO CHANGE N = 10 / 3
        path = "/home/netanelb2/Desktop/Netanel/Research/exiton/harmonic/boson3/bhw60/{}beads/".format(p)   # Harmonic
        print("Path: ", path)
...
##### Boson3 AnHarmonic
# energy_beta = np.array([0, 0, 0, 0, 0, 0, 0])
# stdv_energy_beta = np.array([0, 0, 0, 0, 0, 0, 0])
###### Boson10 AnHarmonic
# energy_beta = np.array([0, 0, 0, 0, 0, 0, 0])
# stdv_energy_beta = np.array([0, 0, 0, 0, 0, 0, 0])
...
#     x_moire_lims = 1*10**13
#     X, Y = np.meshgrid(np.linspace(-x_moire_lims*bohrconv, x_moire_lims*bohrconv, 1024), np.linspace(-x_moire_lims*bohrconv, x_moire_lims*bohrconv, 1024)) # * 10 ** -9  # nm
#     Z = hbaromega + 2 * V_e * (np.cos((-2*pi/(math.sqrt(3)*moire_period))*X - (2*pi/moire_period)*Y - psi) +
#                                np.cos((-2*pi/(math.sqrt(3)*moire_period))*X + (2*pi/moire_period)*Y - psi) +
#                                np.cos(((4*pi)/(math.sqrt(3)*moire_period))*X - psi))
#     # # Plot Moire Potential 2D
